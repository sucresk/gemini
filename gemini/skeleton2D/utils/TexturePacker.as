		...
		public function packTextures(_widthMax:uint, _interval:uint = 2, _verticalSide:Boolean = false, _transparent:Boolean = true):TextureMix {
			...
			for (_i = 0; _i < _len; _i++) {
				...
				if (_texture is DisplayObject) {
					...
					//matrix.a = (_texture as DisplayObject).transform.matrix.a;
					//matrix.b = (_texture as DisplayObject).transform.matrix.b;
					//matrix.c = (_texture as DisplayObject).transform.matrix.c;
					//matrix.d = (_texture as DisplayObject).transform.matrix.d;
				}else {
					//贴图是bitmapData的情况
					_rect = rect;
					_rect.x = 0;
